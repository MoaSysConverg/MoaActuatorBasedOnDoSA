...
이 문서는 사용자 및 개발자가 배포용 exe 대신 **가상환경(`PyMotorEnv_310`)**을 활성화하여 `moa_actuator` GUI 프로그램 및 Jupyter Notebook 예제를 직접 실행하는 절차를 안내합니다.
...
2. **Maxwell(AEDT) 연동 시 gRPC 관련 오류**:
   - Ansys Maxwell 프로그램이 정상적으로 활성화되는지 먼저 확인하고, AEDT를 관리자 권한 등으로 단독 실행한 후 다시 시도해 주십시오. (Ansys 버전은 2026.1을 타겟으로 함)
3. **Dry-run을 통한 사전 확인**:
