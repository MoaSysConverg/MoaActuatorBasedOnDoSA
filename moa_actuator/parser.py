"""Parser for DoSA .dsa (2D) and .dsa3d (3D) design files.

The file format uses ``$begin "BlockName"`` / ``$end "BlockName"`` delimiters
with ``key=value`` lines inside each block.
"""
...
def parse_blocks(text: str) -> RawBlock:
    """Parse the $begin/$end block structure into a tree of RawBlocks."""
    ...
    for raw_line in text.splitlines():
        ...
        if line.startswith("$end"):
            ...
            if stack[-1].name != end_name:
                raise ValueError(
                    f"Mismatched block end: expected {stack[-1].name}, got {end_name}"
                )
            stack.pop()
            continue
...
def _collect_tests(nodes: list[NodeModel]) -> list[TestModel]:
    test_kinds = {
        "Force Test", "Stroke Test", "Current Test",
        "Force-Test", "Stroke-Test", "Current-Test",
    }
    ...
    for node in nodes:
        if node.kind in test_kinds or node.properties.get("KindKey", "").endswith("_TEST"):
            output.append(
                TestModel(name=node.name or node.kind, kind=node.kind, properties=dict(node.properties))
            )
...
def parse_dosa_file(file_path: str | Path) -> DesignModel:
    """Parse a DoSA design file (.dsa or .dsa3d) into a DesignModel."""
    path = Path(file_path)
